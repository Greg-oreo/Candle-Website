...
const Account = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('https://2dfe-66-25-161-126.ngrok-free.app/register_app/login.php', {
        method: 'POST',
        headers: {
      ...
      // Check if the response status is error or success
      if (response.ok) {
        // Handle success
        setSuccessMessage(data.message); // Assuming your PHP returns a message key
      } else {
        // Handle error
        setError(data.message || "Invalid email or password");
      }
    } catch (error) {
      // Handle any network or other fetch errors
